...
class TestThread10A : public osapi::Thread
{
public:
    TestThread10A() : Thread(3, 256, osapi::JOINABLE) { run(); }
private:
    virtual void job() {
...
class TestThread10B : public osapi::Thread
{
public:
    TestThread10B() : Thread(4, 256, osapi::JOINABLE) { run(); }
private:
    virtual void job() {
			  // get initial priority using osapi
...
void TEST_ThreadPriorityChange(void) {
    ...
    printf("Running test 10: thread priority change             ");
